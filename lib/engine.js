...
export function initChart(canvas, config, eras, lang, theme) {
  ...
  function drawEraBands(maxIdx) {
    ...
    for (let i = 0; i <= Math.min(maxIdx, N - 1); i++) {
      if (i === Math.min(maxIdx, N - 1) || (data[i + 1] && data[i + 1].era !== data[i].era)) {
        ...
        const x1 = xPos(startIdx);
        const x2 = (i < N - 1) ? xPos(i + 1) : xPos(i);
        ctx.fillStyle = c;
  ...
  function drawCrosshair(idx) {
    ...
    ctx.fillStyle = hasSecondary ? (cssVar('--series-primary') || '#3b82f6') : eraColor(data[idx].era);
    ctx.fill();
  ...
  function buildStats() {
    ...
    if (hasSecondary) {
      ...
      statsBar.innerHTML = `
        <div class="stat-card"><div class="stat-label">${esc(t('current'))}</div><div class="stat-value" style="color:var(--series-primary)">${esc(last.value.toFixed(config.decimals))}</div><div class="stat-unit">${esc(last.label)}</div></div>
        <div class="stat-card"><div class="stat-label">${esc(t('lowest'))}</div><div class="stat-value" style="color:var(--series-secondary)">${esc(last.value2.toFixed(config.decimals))}</div><div class="stat-unit">${esc(last.label)}</div></div>
  ...
  function buildEvents() {
    ...
    if (!Array.isArray(config.events) || config.events.length === 0) {
      ...
      if (src?.name) {
        eventsGrid.innerHTML = `<div class="event-card" data-idx="-1" style="cursor:default"><div class="e-pip" style="background:var(--series-primary)"></div><div><div class="e-label">Official source</div><div class="e-text">${esc(src.name)}${src.table ? ` — ${esc(src.table)}` : ''}${src.url ? ` · <a href="${safeHref(src.url)}" target="_blank" rel="noreferrer">link</a>` : ''}</div></div></div>`;
      } else {
    ...
    eventsGrid.innerHTML = config.events.map(ev => {
      ...
      return `<div class="event-card" data-idx="${esc(String(ev.idx))}"><div class="e-pip" style="background:${esc(c)}"></div><div><div class="e-label">${esc(d.label)} · ${esc(eraName)}</div><div class="e-text">${esc(txt)}</div></div></div>`;
    }).filter(Boolean).join('');
