...
export function initChart(canvas, config, eras, lang, theme) {
  ...
  function showTooltip(idx) {
    ...
    if (hasSecondary) {
      tooltipEl.innerHTML = `<div class="t-date">${d.label}</div><div class="t-price" style="color:#3b82f6">${seriesLabelPrimary()}: ${d.value.toFixed(config.decimals)}</div><div class="t-price" style="color:#22c55e">${seriesLabelSecondary()}: ${d.value2.toFixed(config.decimals)}</div><div class="t-era"><span class="pip" style="background:#3b82f6"></span>${seriesLabelPrimary()} / <span class="pip" style="background:#22c55e"></span>${seriesLabelSecondary()}</div>`;
    } else {
      tooltipEl.innerHTML = `<div class="t-date">${d.label}</div><div class="t-price">${d.value.toFixed(config.decimals)} ${config.unit}</div><div class="t-era"><span class="pip" style="background:${c}"></span>${eras[d.era] ? eras[d.era].name : d.era}</div><div class="t-pm">PM: ${d.pm}</div>`;
    }
    let left = px + 16;
  ...
  function buildStats() {
    ...
    if (hasSecondary) {
      ...
      statsBar.innerHTML = `
        <div class="stat-card"><div class="stat-label">${t('current')}</div><div class="stat-value" style="color:#3b82f6">${last.value.toFixed(config.decimals)}</div><div class="stat-unit">${last.label}</div></div>
        <div class="stat-card"><div class="stat-label">${t('lowest')}</div><div class="stat-value" style="color:#22c55e">${last.value2.toFixed(config.decimals)}</div><div class="stat-unit">${last.label}</div></div>
        <div class="stat-card change"><div class="stat-label">${t('peak')}</div><div class="stat-value">${bCh > 0 ? '+' : ''}${bCh}%</div><div class="stat-unit">${config.timeRange || ''}</div></div>
        <div class="stat-card change"><div class="stat-label">${t('change')}</div><div class="stat-value">${gCh > 0 ? '+' : ''}${gCh}%</div><div class="stat-unit">${config.timeRange || ''}</div></div>`;
      return;
    }
  ...
  function buildLegend() {
    ...
    legendEl.innerHTML = Object.entries(eras).map(([k, v]) =>
      `<div class="legend-item"><span class="ldot" style="background:${eraColor(k)}"></span>${v.name}</div>`
    ).join('');
  }
  ...
  function buildExtraWidget() {
    ...
    const rows = limited.map((r) => {
      ...
      return `<div class="yw-row"><div class="yw-year">${r.year}</div><div>${usd}M</div><div>${eur}M</div><div><b>${totalUsd}M</b></div></div>`;
    }).join('');
  ...
  function buildEvents() {
    ...
    if (!Array.isArray(config.events) || config.events.length === 0) {
      ...
      if (src?.name) {
        eventsGrid.innerHTML = `<div class="event-card" data-idx="-1" style="cursor:default"><div class="e-pip" style="background:#3b82f6"></div><div><div class="e-label">Official source</div><div class="e-text">${src.name}${src.table ? ` — ${src.table}` : ''}${src.url ? ` · <a href="${src.url}" target="_blank" rel="noreferrer">link</a>` : ''}</div></div></div>`;
      } else {
    ...
    eventsGrid.innerHTML = config.events.map(ev => {
      ...
      return `<div class="event-card" data-idx="${ev.idx}"><div class="e-pip" style="background:${c}"></div><div><div class="e-label">${d.label} · ${eraName}</div><div class="e-text">${txt}</div></div></div>`;
    }).filter(Boolean).join('');
