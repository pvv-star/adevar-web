...
export function initChart(canvas, config, eras, lang, theme) {
  ...
  function showTooltip(idx) {
    ...
    if (hasSecondary) {
      tooltipEl.innerHTML = `<div class="t-date">${d.label}</div><div class="t-price" style="color:#3b82f6">${t('boys')}: ${d.value.toFixed(config.decimals)}</div><div class="t-price" style="color:#22c55e">${t('girls')}: ${d.value2.toFixed(config.decimals)}</div><div class="t-era"><span class="pip" style="background:#3b82f6"></span>${t('boys')} / <span class="pip" style="background:#22c55e"></span>${t('girls')}</div>`;
    } else {
  ...
  function buildLegend() {
    ...
    if (hasSecondary) {
      legendEl.innerHTML = `
        <div class="legend-item"><span class="ldot" style="background:#3b82f6"></span>${t('boys')}</div>
        <div class="legend-item"><span class="ldot" style="background:#22c55e"></span>${t('girls')}</div>`;
      return;
    }
