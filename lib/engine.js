...
export function initChart(canvas, config, eras, lang, theme) {
  ...
  function buildEvents() {
    ...
    if (!Array.isArray(config.events) || config.events.length === 0) {
      ...
      if (src?.name) {
        eventsGrid.innerHTML = `<div class="event-card" data-idx="-1" style="cursor:default"><div class="e-pip" style="background:var(--series-primary)"></div><div><div class="e-label">Official source</div><div class="e-text">${esc(src.name)}${src.table ? ` — ${esc(src.table)}` : ''}${src.url ? ` · <a href="${esc(src.url)}" target="_blank" rel="noreferrer">link</a>` : ''}</div></div></div>`;
      } else {
