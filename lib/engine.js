...
export function initChart(canvas, config, eras, lang, theme) {
  ...
  function drawGrid() {
    ctx.strokeStyle = cssVar('--grid') || '#e0e3ea';
    ctx.lineWidth = 0.5;
    ctx.font = '11px "Inter",sans-serif';
    ctx.fillStyle = cssVar('--text3') || '#5f6680';
    ctx.textAlign = 'right';
  ...
  function drawLine(prog) {
    ...
    const fillColor = cssVar('--line-fill') || 'rgba(30,63,110,0.06)';
    ...
    if (maxI < N - 1 && frac > 0) {
      ...
      const vE = data[maxI].value + frac * (data[maxI + 1].value - data[maxI].value);
    ...
    for (let i = 0; i <= maxI && i < N; i++) {
      if (i === 0) ctx.moveTo(xPos(i), yPos(data[i].value));
      else ctx.lineTo(xPos(i), yPos(data[i].value));
    }
    if (maxI < N - 1 && frac > 0) {
      const xE = xPos(maxI) + frac * (xPos(maxI + 1) - xPos(maxI));
      const vE = data[maxI].value + frac * (data[maxI + 1].value - data[maxI].value);
  ...
  function drawCrosshair(idx) {
    ...
    ctx.fillStyle = eraColor(data[idx].era);
    ctx.fill();
  ...
  setTimeout(() => {
    ...
    const replayBtn = wrap?.querySelector('#replayBtn') || document.getElementById('replayBtn');
    const speedBtn = wrap?.querySelector('#speedBtn') || document.getElementById('speedBtn');
    if (replayBtn) {
      replayBtn.addEventListener('click', () => api.replay());
    }
    if (speedBtn) {
      speedBtn.addEventListener('click', function () { api.toggleSpeed(this); });
    }
  }, 0);
