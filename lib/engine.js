...
import { formatYLabel, formatValue } from './chart-utils.js';
...
export function initChart(canvas, config, eras, lang, theme) {
  ...
  function drawEraBands() {
    // Political era background bands disabled by product decision.
  }
  ...
  function drawGrid() {
    ...
    for (let i = 0; i <= config.gridSteps; i++) {
      ...
      ctx.stroke();
      ctx.fillText(formatYLabel(v, config.decimals), PADDING.left - 10, y);
    }
  ...
  function drawLine(prog) {
    ...
    if (hasSecondary) {
      drawSeries(prog, 'value2', cssVar('--series-secondary') || '#22c55e', false);
    }
  ...
  function drawCrosshair(idx) {
    ...
    ctx.fillStyle = hasSecondary ? (cssVar('--series-primary') || '#3b82f6') : (cssVar('--series-primary') || '#3b82f6');
    ctx.fill();
    ...
    if (hasSecondary) {
      ...
      ctx.fillStyle = cssVar('--series-secondary') || '#22c55e';
      ctx.fill();
      ctx.strokeStyle = isDark() ? '#1a1f2e' : '#fff';
      ctx.lineWidth = 2;
      ctx.stroke();
  ...
  function showTooltip(idx) {
    ...
    if (hasSecondary) {
      tooltipEl.innerHTML = `<div class="t-date">${esc(d.label)}</div><div class="t-price" style="color:var(--series-primary)">${esc(seriesLabelPrimary())}: ${esc(formatValue(d.value, config.decimals))}</div><div class="t-price" style="color:var(--series-secondary)">${esc(seriesLabelSecondary())}: ${esc(formatValue(d.value2, config.decimals))}</div><div class="t-era"><span class="pip" style="background:var(--series-primary)"></span>${esc(seriesLabelPrimary())} / <span class="pip" style="background:var(--series-secondary)"></span>${esc(seriesLabelSecondary())}</div>`;
    } else {
      tooltipEl.innerHTML = `<div class="t-date">${esc(d.label)}</div><div class="t-price">${esc(formatValue(d.value, config.decimals))} ${esc(config.unit)}</div>`;
    }
    let left = px + 16;
  ...
  function animLoop(ts) {
    ...
    if (pricePillEl && nearIdx >= 0 && nearIdx < N) {
      ...
      const sx = cr.width / cw();
      const sy = cr.height / ch();
      pricePillEl.textContent = formatValue(data[nearIdx].value, config.decimals) + ' ' + config.unit;
  ...
  function buildStats() {
    ...
    if (hasSecondary) {
      ...
      statsBar.innerHTML = `
        <div class="stat-card"><div class="stat-label">${esc(t('current'))}</div><div class="stat-value" style="color:var(--series-primary)">${esc(formatValue(last.value, config.decimals))}</div><div class="stat-unit">${esc(last.label)}</div></div>
        <div class="stat-card"><div class="stat-label">${esc(t('lowest'))}</div><div class="stat-value" style="color:var(--series-secondary)">${esc(formatValue(last.value2, config.decimals))}</div><div class="stat-unit">${esc(last.label)}</div></div>
    ...
    statsBar.innerHTML = `
      <div class="stat-card"><div class="stat-label">${esc(t('current'))}</div><div class="stat-value">${esc(formatValue(s.current, config.decimals))}</div><div class="stat-unit">${esc(config.unit)}</div></div>
      <div class="stat-card"><div class="stat-label">${esc(t('lowest'))}</div><div class="stat-value">${esc(formatValue(s.lowest, config.decimals))}</div><div class="stat-unit">${esc(config.unit)}</div></div>
      <div class="stat-card"><div class="stat-label">${esc(t('peak'))}</div><div class="stat-value">${esc(formatValue(s.peak, config.decimals))}</div><div class="stat-unit">${esc(config.unit)}</div></div>
  ...
  function buildLegend() {
    ...
    if (hasSecondary) {
      legendEl.innerHTML = `
        <div class="legend-item"><span class="ldot" style="background:var(--series-primary)"></span>${esc(seriesLabelPrimary())}</div>
        <div class="legend-item"><span class="ldot" style="background:var(--series-secondary)"></span>${esc(seriesLabelSecondary())}</div>`;
      return;
    }
  ...
  function buildExtraWidget() {
    ...
    const title = esc(t('yearlyTotalsTitle') || 'Yearly totals');
    const usdLabel = esc(t('usdLabel') || 'USD');
    const eurLabel = esc(t('eurLabel') || 'EUR');
    const totalLabel = esc(t('totalUsdLabel') || 'Total (USD)');
    ...
    const rows = limited.map((r) => {
      ...
      return `<div class="yw-row"><div class="yw-year">${esc(String(r.year))}</div><div>${esc(usd)}M</div><div>${esc(eur)}M</div><div><b>${esc(totalUsd)}M</b></div></div>`;
    }).join('');

    const athYear = esc(String(ath?.year ?? '—'));
    const athValue = esc(Number(ath?.totalUsd || 0).toFixed(1));
  ...
  function buildEvents() {
    ...
    if (eventsTitle) eventsTitle.textContent = '';
  ...
  function isTouchInPlot(touch) {
    ...
    return (
      cx >= PADDING.left && cx <= cw() - PADDING.right &&
      cy >= PADDING.top && cy <= PADDING.top + plotH()
    );
  }
  ...
  function onTouchStart(e) {
    if (e.touches.length === 1 && isTouchInPlot(e.touches[0])) {
      e.preventDefault();
      handleTouch(e);
    }
  }
