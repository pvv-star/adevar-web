...
export function formatIndicatorUpdate(changes) {
  ...
  const lines = changes.map((c) => {
    ...
    return `🔹 ${escTg(c.name)}: ${fmt(c.value)} ${escTg(c.unit)}${delta}`;
  });
...
export function formatDailyDigest(data) {
  ...
  lines.push(`❌ Surse cu erori: ${uniqueErrSources.length ? uniqueErrSources.map(escTg).join(', ') : 'niciuna'}`);
  lines.push('');
  ...
  if (data.topNews.length) {
    ...
    for (const n of data.topNews.slice(0, 5)) {
      ...
      lines.push(`🔹 [${score}] ${escTg(n.title)} <i>(${escTg(n.source_slug)})</i>`);
    }
    lines.push('');
  }
  ...
  if (data.indicatorChanges.length) {
    ...
    for (const c of data.indicatorChanges) {
      const delta = pctChange(c.oldValue, c.value);
      lines.push(`🔹 ${escTg(c.name)}: ${fmt(c.value)} ${escTg(c.unit)}${delta}`);
    }
    lines.push('');
  }
